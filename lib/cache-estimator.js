...
function estimateCacheTokens({ usage, model, upstream, sessionId, queryDb, systemPromptTokens }) {
  ...
  // Skip estimation for calls without a session_id. Claude Code sessions always
  // send X-Claude-Code-Session-Id. Internal /ask and /judge calls are single-turn
  // with no prompt caching — estimating cache for them inflates cache_write_tokens.
  if (!sessionId) {
    return { usage, estimated: false };
  }
  ...
  if (queryDb) {
    try {
      // Sanitize sessionId to UUID format only (防 SQL injection)
      const safeId = String(sessionId).replace(/[^a-zA-Z0-9-]/g, '');
      if (safeId.length > 0 && safeId.length <= 64) {
        const rows = queryDb(
